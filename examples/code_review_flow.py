...
Full review cycle: create task → request review → add file-anchored
comments → give verdict → approve → merge.
...
def main():
    ...
    client.post(f"/reviews/{review['id']}/comments", json={
        ...
        "line_number": 15,
        "body": "This uses a fixed window counter. Consider using a sliding window "
                "algorithm — fixed windows can allow 2x the rate at window boundaries."
    })
    ...
    client.post(f"/reviews/{review['id']}/comments", json={
        "file_path": "middleware/rate_limiter.py",
        "line_number": 42,
        "body": "Good use of Redis MULTI/EXEC for atomicity. But add a TTL to the key "
                "to prevent orphaned entries if Redis restarts."
    })
    ...
    client.post(f"/reviews/{review['id']}/comments", json={
        ...
        "line_number": 1,
        "body": "Tests look solid. Add one more: test behavior when Redis is down "
                "(should fail open or return 503?)."
    })
    ...
    client.post(f"/reviews/{review2['id']}/verdict", json={
        "verdict": "approve",
        "summary": "Sliding window looks correct. TTL is in place. "
                   "Failure test covers the edge case. Ship it."
    })
    print(f"\nVerdict: approve ✓")
    print(f"  'Ship it.'")
